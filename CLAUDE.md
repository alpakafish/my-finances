...
## «Нал.»-операции (excluded_from_total)

Операция (обычно доход) может быть помечена флагом `excluded_from_total`
(`transactions.excluded_from_total`, чекбокс «Нал., не в общей сумме» в форме
операции) — она получает бейдж «нал.» и не входит в плоские итоги по типу без
разбивки на категории (шапка «Доход»/«Расход»/«Баланс» за месяц —
`routes/summary.js` `/:month`; «Итоги по годам» — `/yearly-totals`; итоговые
Доход/Расход/Баланс в Excel-экспорте — `routes/export.js` `totalStmt`), но
**остаётся** в суммах по категориям (`categoryBreakdown()`, `/overview`,
`/yearly`, поколоночные суммы `sumStmt` в экспорте) — то есть на графиках и в
разбивке по категориям. Из-за этого график/разбивка и плоский итог за один и
тот же период могут законно показывать разные числа — это по дизайну, не баг
(есть поясняющие подписи на Дашборде и на вкладке «По годам»).

При добавлении новых мест, где считается сумма операций, определи, к какой
категории оно относится (плоский итог → фильтровать `excluded_from_total = 0`;
разбивка по категориям/график → не фильтровать), и держи это в голове — та же
развилка, что с `rollup_id`, только по другой оси.

**Готча, пойманная при реализации:** список лет для таблицы «Итоги по годам»
(`/yearly-totals`) нельзя выводить из уже отфильтрованного `excluded_from_total
= 0` запроса — год, где были только операции «нал.», тогда пропадал бы из
таблицы целиком вместо строки 0/0/0 (проверено вручную: год с одной
«нал.»-операцией и больше ничем). Список лет там теперь берётся отдельным
незафильтрованным запросом (как в `/years`), а фильтр применяется только к
самим суммам.

## Фичи, затрагивающие Excel-импорт/экспорт или существующие операции

Это самая опасная категория изменений в проекте — единственная база данных
пользователя (`data/smeta.db`) не резервируется автоматически ничем, кроме
встроенного бэкапа (`backup.js`), и любая порча тут необратима. Прежде чем
мержить фичу, которая трогает `routes/import.js`, `routes/export.js`, схему
`transactions`/`categories`/`app_settings` в `db.js`, или как-либо массово
меняет уже сохранённые операции — проверить это explicitly, не полагаясь на
то, что код "выглядит правильным" на пустой тестовой БД:

- **Реальный round-trip, а не только happy path.** Экспортировать тестовый
  набор данных, импортировать обратно — и в пустую БД, и в БД, где уже
  что-то есть — сверить количество операций и суммы до/после.
- **Обратная совместимость файлов.** Новые колонки в экспорте добавляются
  только в конец (см. прецедент с «Метка»/«Повтор.» — искать по коду), иначе
  файл, экспортированный до фичи, перестанет нормально импортироваться после
  неё, и наоборот: у пользователя, который сделал бэкап месяц назад, при
  восстановлении что-то отвалится.
- **Проверять на «грязных» данных, а не только на чистых**: операции с
  флагом «нал.» (см. выше), рекуррентные шаблоны, дубликаты, неизвестные или
  уже удалённые категории — именно такие граничные случаи в этом проекте
  реально ломались (см. регрессии в `TESTING.md`).
- **Изменения схемы (`db.js`) — прогнать на реальном, непустом дампе**,
  скопированном в изолированный `DATA_DIR`, а не только на свежесозданной
  тестовой БД: миграция, которая тихо роняет или обнуляет существующие
  строки, не всегда видна на пустых фикстурах.
- **`delete-all-data`/полный сброс.** При добавлении нового хранилища (новая
  таблица, новый ключ в `app_settings`) сразу и явно решить, должен ли он
  чиститься при полном сбросе, и проверить это тестом — про это легко
  забыть, если не спросить целенаправленно (прецедент:
  `overall_monthly_budget`/`overall_budget_notification_dismissals`).

Как и везде в этом файле: только на изолированном `DATA_DIR=/tmp/...`,
...
## Обращение к внешним сервисам (прецедент — `routes/vacation.js`)

До калькулятора отпускных (2026-08-19) приложение никогда не ходило наружу
из самой фичи — только electron-updater за обновлениями. README и раздел
«Безопасность» прямо обещали «ничего не отправляется во внешние сервисы»,
и калькулятор отпускных (обращается к открытому производственному
календарю isdayoff.ru за праздниками/переносами выходных) — первое
исключение. Если добавляешь ещё одну такую интеграцию:

- **Обнови README** (список функций и раздел «Безопасность») честно — не
  оставляй старую формулировку «ничего не уходит», раз это уже не так.
  Явно напиши, какие именно данные уходят (в случае отпускных — только
  диапазон дат, без сумм).
- **Деградируй, а не падай, если сервис недоступен.** `routes/vacation.js`
  при неудаче `fetch` тихо переключается на `fallbackCalendar()` (только
  выходные по дням недели, без праздников) вместо 500 на весь расчёт —
  фронтенд получает `calendarAvailable: false` и явно предупреждает
  пользователя, что праздники не учтены, а не молча показывает неточную
  цифру как точную.
- **Отдели чистую логику от похода в сеть**, если она есть — `calculateVacation()`
  не знает про `fetch` вообще, принимает уже готовый календарь. Это не
  только чище, но и снимает необходимость в сетевых тестах для всей логики
  сразу: тесты гоняют чистую функцию на заранее собранном фикстур-календаре
  той же формы, что даёт реальный API, и только один тест бьёт по-настоящему
  в live-сервис (см. TESTING.md).
