...
- Учёт операций: добавление, просмотр по месяцам с фильтром «Все/Расходы/Доходы», изменение (дата, тип, категория, сумма, заметка — прямо в списке, кнопка ✎), удаление с возможностью отменить (кнопка в уведомлении или Cmd/Ctrl+Z)
...
Не реализовано (возможные следующие шаги): мультивалютность, бюджеты/лимиты по обычным категориям с уведомлениями (сейчас лимиты есть только у целей накопления).
...
Данные хранятся в `~/Library/Application Support/My Finances/` — это стандартное место для данных приложений на Mac, обновления приложения его не затрагивают. Когда выйдет новая версия, приложение сообщит об этом и предложит открыть страницу загрузки — скачайте новый `.dmg` и перетащите поверх старой версии в Applications, так же, как при первой установке (автоматическая установка обновления пока недоступна).
