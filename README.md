...
Локальный учёт личных расходов и доходов — веб-страница или приложение для Mac, на выбор. Все данные хранятся только на вашем компьютере — ничего не уходит в облако.
...
> Apple Silicon или Intel? См. [«Приложение для Mac»](#приложение-для-mac) ниже.
...
- **Обработка**: операции сохраняются локально; категории можно заводить, переименовывать, перекрашивать и удалять (с переносом операций на другую категорию, если категория уже использовалась).
- **Выход**: отчёт за выбранный месяц (доход / расход / баланс, разбивка расходов по категориям, круговая диаграмма структуры расходов), графики по месяцам и по годам, и экспорт всех данных в Excel одной кнопкой.
...
- **Приложение для Mac** — то же самое, но отдельной программой, без браузера и без терминала.

В обеих версиях одинаковые вкладки: Дашборд, Операции, Категории, Цели, По годам и Настройки, плюс кнопки «Импорт»/«Экспорт в Excel» — этим же файлом `.xlsx` удобно переносить данные между устройствами или делать резервную копию.

## Скриншоты

> Данные на скриншотах — демонстрационные, не настоящий бюджет.
...
**Операции** — добавление операции и список за месяц с фильтром по типу; удалённую по ошибке операцию можно вернуть кнопкой «Отменить» или сочетанием Cmd/Ctrl+Z:
...
- Учёт операций: добавление, просмотр по месяцам с фильтром «Все/Расходы/Доходы», удаление с возможностью отменить (кнопка в уведомлении или Cmd/Ctrl+Z)
...
- Настройки: полная очистка данных (с подтверждением) и защита приложения паролем/Touch ID (в приложении для Mac)
...
Не реализовано (возможные следующие шаги): редактирование уже добавленной операции (сейчас — удаление и повторное добавление), мультивалютность, бюджеты/лимиты по обычным категориям с уведомлениями (сейчас лимиты есть только у целей накопления).
...
3. **При первом запуске macOS может показать «Apple не может проверить,
   ...
   пока не подписана платным сертификатом Apple Developer. Приложение при
   этом безопасно, просто Apple его не сертифицировала. Чтобы всё же
   открыть — один раз, дальше запускается как обычно:
...
Данные хранятся в `~/Library/Application Support/My Finances/` — это стандартное место для данных приложений на Mac, обновления приложения его не затрагивают. Когда выйдет новая версия, приложение само предложит обновиться.
...
**Защита паролем/Touch ID** (Настройки → «Защищать приложение паролем», доступно в приложении для Mac): если включить, при каждом запуске нужно будет подтвердить личность, прежде чем откроется доступ к данным.
...
Это удобная защита от чужого случайного доступа к приложению на вашем компьютере, а не банковский уровень шифрования — для личного финансового трекера этого достаточно.
