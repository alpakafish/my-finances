...
**Операции** — добавление операции и список за месяц с фильтром по типу; любую операцию можно изменить (✎) или удалить, удалённую по ошибке — вернуть сразу кнопкой «Отменить» или сочетанием Cmd/Ctrl+Z, а позже — из Корзины внизу вкладки, которая хранит последние 10 удалений:
...
- Учёт операций: добавление, просмотр по месяцам с фильтром «Все/Расходы/Доходы» и по категории, изменение (дата, тип, категория, сумма, заметка — прямо в списке, кнопка ✎), удаление с возможностью отменить (кнопка в уведомлении, Cmd/Ctrl+Z или Корзина — хранит последние 10 удалений и переживает перезапуск приложения; операции постарше можно найти в автоматическом бэкапе). Суммы в списке операций показываются с копейками (в остальных местах — Дашборд, Лимиты, Цели, По годам — округлённо, для более чистого вида агрегированных сумм)
...
- Повторяющиеся операции — отметьте «Повтор.» для зарплаты, аренды, подписок; при открытии нового месяца, где такой операции ещё нет, появится подсказка добавить её снова (сумму можно поправить) или пропустить на этот месяц. Для платежей с неизменной суммой можно дополнительно отметить «Не спрашивая» — тогда операция добавится сама, без подсказки
- Категории расходов и доходов: добавление, переименование, смена цвета, удаление с переносом операций
- Отчёт за месяц с диаграммами — расходы по категориям (строку со свёрнутой в неё целью можно раскрыть кликом и увидеть разбивку), структура расходов (с возможностью скрывать категории на диаграмме), расходы и доходы по месяцам за текущий год
- Сверка с картой — необязательная точка отсчёта (дата и сумма) в Настройках; на Дашборде появляется карточка с накопительным остатком с этой даты и сравнением с тем, что реально на карте, плюс кнопка перенести точку отсчёта на сегодня после сверки
...
- Общий бюджет на месяц — необязательный лимит на все расходы сразу, а не на отдельную категорию; прогресс на Дашборде, уведомление — тот же принцип, что у лимитов категорий
...
- Импорт и экспорт в Excel — включая перенос данных между устройствами и бэкап через собственный формат экспорта. Повторный импорт «умный»: операция, отредактированная на одном устройстве, при переносе на другое обновится, а не задвоится (удаления между устройствами так не переносятся — для этого нужен уже настоящий сервер). В Настройках отдельно можно выгрузить только период (например, год для бухгалтерии) — не влияет на обычную кнопку «Экспорт в Excel» и на автоматические бэкапы, которые всегда полные
...
- Расчёт отпускных (только по ТК РФ) — упрощённо, по окладу (не по среднему заработку за 12 месяцев), с указанием дат отпуска и чисел выплат в месяце. Показывает сумму отпускных за 3 календарных дня до начала отпуска и суммы ближайших выплат с учётом отпуска, автоматически проверяя праздники и перенос выплаты, если расчётная дата попадает на выходной
- Прогноз бюджета — черновик на будущий месяц: планируемый доход, суммы по категориям расходов и разовая покупка, которую хочется проверить («хватит ли на новую клавиатуру»); остаток считается сразу. Черновик сохраняется отдельно от операций и никак на них не влияет — можно менять сколько угодно раз, ничего не создаёт и не портит в реальных данных
- Гид по приложению — кнопка «?» в правом верхнем углу показывает обзор возможностей ещё раз
- Локальное хранение данных — операции, категории, цели и настройки не покидают ваш компьютер (исключение — калькулятор отпускных, см. «Безопасность» ниже)
...
Приложение не собирает и не отправляет ваши финансовые данные никуда — операции, категории, цели и суммы хранятся только на вашем компьютере. Импорт/экспорт в Excel работает полностью офлайн. Единственное исключение — необязательный калькулятор отпускных (вкладка «Отпускные»): чтобы учесть праздники и переносы выходных, он обращается к открытому производственному календарю [isdayoff.ru](https://isdayoff.ru/), но передаёт туда только диапазон дат — без оклада, сумм или какой-либо другой личной информации. Секретов вроде паролей от сторонних сервисов в приложении нет.
