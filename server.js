...
// Страховка на случай будущего async-роута без try/catch (как был найден и
// исправлен в routes/export.js) — Express 4 не ловит отклонённые promise'ы
// async-хендлеров сам, а с Node 15+ необработанный unhandledRejection по
// умолчанию завершает процесс целиком. Каждый async-роут всё равно должен
// сам оборачивать рискованный код в try/catch (это единственный способ
// ответить пользователю осмысленным 500 вместо оборванного соединения) — это
// именно страховка, а не замена точечной обработке.
